...
Uses Ollama Cloud HTTP API. The LLM receives retrieved project evidence,
...
def ollama_status() -> tuple[bool, str]:
    """Check whether the Ollama Cloud API is reachable."""
    if not OLLAMA_API_KEY:
        return False, "Ollama API key is not configured."
    ...
    try:
        ...
        return True, f"Connected to Ollama Cloud. Model: {OLLAMA_MODEL}"

    except requests.RequestException:
        return False, "Ollama Cloud could not be reached."
...
def _format_context(context: dict[str, Any]) -> str:
    ...
    if context.get("relationships"):
        ...
        for r in context["relationships"]:
            ...
            lines.append(
                f"- {r.get('source_entity_text')} "
                f"[{r.get('source_entity_type')}] "
                f"--{r.get('relationship_type')}--> "
                f"{r.get('target_entity_text')} "
                f"[{r.get('target_entity_type')}] | "
                f"Evidence {r.get('evidence_id')} | "
                f"Record/Page {loc} | "
                f"Source {r.get('source_filename')}"
            )
...
def ask_ollama(question: str, context: dict[str, Any]) -> str:
    ...
    if not OLLAMA_API_KEY:
        return "Ollama API key is not configured."
    ...
    try:
        ...
        if not answer:
            return "The Ollama Cloud model returned an empty response."
